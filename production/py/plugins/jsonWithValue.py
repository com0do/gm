...
class JtoXPlugin(plugin.PyangPlugin):
    def __init__(self):
        ...
        self.label = ["plato-labelling"]
        self.withDefault = 0
        self.noDefault = 0
        self.withMandatory = 0
        self.noMandatory = 0
    def add_output_format(self, fmts):
        self.multiple_modules = True
        fmts['jsonWithValue'] = self

    def setup_fmt(self, ctx):
        ctx.implicit_errors = False
        if ctx.opts.label:
            self.label = ctx.opts.label

    def add_opts(self, optparser):
        optlist = [
            optparse.make_option("--label", action="append",
                                 help="read nokia specific label ",)
            ]
    ...
    def process_children(self, node, parent, pmod):
        ...
        for ch in node.i_children:
            ...
            if ch.keyword == "container":
                ...
                if default:
                    default = str(default).split()[-1]
                ndata.append(default)
                mandatory = ch.search_one("mandatory")
                if mandatory:
                    mandatory = str(mandatory).split()[-1]
                ndata.append(mandatory)
                if self.label:
                    label_str = ''
                    for i,l in enumerate(self.label):
                        if i > 0:
                            label_str += '|'
                        for substmt in ch.substmts:
                            if l in substmt.keyword:
                                label_str += substmt.arg
                                break
                    ndata.append(label_str)
